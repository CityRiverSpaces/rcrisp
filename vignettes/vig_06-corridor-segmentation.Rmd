...
By default, the `delineate()` function returns a `delineation` object with the corridor delineation only (i.e., `segments = FALSE` and `riverspace = FALSE`). The corridor can be segmented either by setting the argument `segments = TRUE` in `delineate()` (or in `delineate_city_river()`), or by using the `delineate_segments()` function in a separate step.
