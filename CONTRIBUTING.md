...
This outlines how to propose a change to rcrisp. For a detailed
discussion on contributing to packages, please see the tidyverse
...
- Fork the package and clone onto your computer. If you haven’t done
  ...
  `usethis::create_from_github("CityRiverSpaces/rcrisp", fork = TRUE)`.
...
We strive to develop `rcrisp` into a robust tool for delineation.
Although the package already produces reliable delineations, we do see
room for improvement.

We plan to:

- Expand the use of input data from OpenStreetMap to Overture Maps and
  from Copernicus GLO-30 Digital Elevation Models to Global Digital
  Terrain Models from OpenTopography.
- Improve segmentation (1) by merging neighbouring corridor segments
  using morphological similarity learned from segment-level
  morphometrics through clustering and (2) by splitting large segments
  based on potential river crossings identified in the street network.
- Improve the user interface of `rcrisp` and validate its output while
  building an inventory of use cases to meet the needs of a broad
  research community.

We will not:

- Develop `rcrisp` into a package for urban river space analysis, but
  maintain its focus on providing morphologically grounded boundaries to
  be adopted in subsequent analyses using other domain-specific tools.

## Code of Conduct

Please note that the rcrisp project is released with a [Contributor Code
of
Conduct](https://cityriverspaces.github.io/rcrisp/CODE_OF_CONDUCT.md).
